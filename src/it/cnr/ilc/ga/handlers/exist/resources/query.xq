...
if ($fx eq 'form')  then
	if($px eq 'ANY') then  
		$node/../w[ft:query(@form,$tx)]
	else
		$node/../w[ft:query(@form,$tx) and ft:query(./@pos,$px)]
...
for $couple in $couples
	let $pericg:=for $wg in $couple/field[ft:query(@name,'analysis_gr')]/w
			return if ($colorg=$wg/@id) then <span class='hlg' style='color:red' title='{$wg/@lemma/xs:string(.)}'> {$wg/@form/xs:string(.)} </span> else $wg/@form/xs:string(.)
...
return 
	<div class='couple'><p>
		<div class='pericg'><b> {$couple/field[ft:query(@name,'info_gr')]/text()} </b>{$pericg}</div><br/>
		<div class='perica'><b> {$couple/field[ft:query(@name,'info_ar')]/text()} </b>{$perica}</div><br/>
	</p></div>
}</div>